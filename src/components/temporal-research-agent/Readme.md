...
The complete historical implementation is captured in
[`before/phaseE_workflow.js`](/temporal-research-agent/code/before/). Its SHA-256 matches
the original file recorded in [`before/provenance.json`](/temporal-research-agent/before/provenance.json).
...
The episode and series definitions live in
[`podcast_preset.py`](/temporal-research-agent/code/inputs/). All ten original
episode briefs, seed papers, and frontier flags are represented as typed
dataclasses. The detailed editorial requirements live in
[`podcast_prompts.py`](/temporal-research-agent/code/prompts/).

The Workflow in
[`podcast_workflow.py`](/temporal-research-agent/code/workflow/) retains the
...
[`podcast_activities.py`](/temporal-research-agent/code/activities/) contains
five Activities:

| Activity | External work | Durable output |
|---|---|---|
| `research_episode` | calls SciX and Code Intelligence Digest in live mode, reads fixtures in recording mode, reads the clock, writes evidence | research Markdown, evidence index, source IDs, hashes, and artifact references |
| `write_deep_dive` | invokes the configured writer in live mode and reads/writes artifacts | deep-dive reference |
| `write_podcast_script` | invokes the writer and renders the episode script | script reference |
| `write_series_review` | reads completed deep dives and synthesizes a review | series-review reference and included episode keys |
| `write_pipeline_manifest` | reads result metadata and writes the final index | compact `PodcastPipelineResult` |

The live MCP calls use a stable logical request ID and a write-once response
...
[`podcast_worker.py`](/temporal-research-agent/code/worker/) connects to the
Temporal Service, polls the `temporal-podcast-research` task queue, and
registers one Workflow with all five Activities:
...
- [Multi-Agent Orchestration literature review](/temporal-research-agent/before/products/multiagent-orchestration/20-literature-review.md)
  and five podcast scripts;
- [Code Retrieval & Enterprise Codebases literature review](/temporal-research-agent/before/products/code-retrieval/20-literature-review.md)
  and five podcast scripts.

Those files show the useful product that existed before Temporal.

The full fixture-backed Temporal Workflow then exercised all ten episode
branches and completed with:

- 10 research artifacts;
- 10 deep dives;
- 10 podcast scripts;
- 2 series literature reviews;
- 20 source records split across SciX-shaped and Code Intelligence
  Digest-shaped evidence;
- 0 failed episodes;
- 1 provenance manifest.

Read the Temporal-produced
[Multi-Agent Orchestration review](/temporal-research-agent/after/fixture-products/reviews/mas-literature-review.md),
[Code Retrieval review](/temporal-research-agent/after/fixture-products/reviews/code-literature-review.md),
or [manifest](/temporal-research-agent/after/fixture-products/manifest.json). The fixture outputs are
short demonstration editions. The live writer path carries the full original
prompt contracts.
...
[Watch the recording](/temporal-research-agent/demo/out/temporal-literature-review-demo.mp4) or inspect
the [recording manifest](/temporal-research-agent/demo/out/recording-manifest.json). The video shows the
...
[`recording.py`](/temporal-research-agent/src/durable_research/recording.py), and the run metadata is in
[`demo/out/run-artifacts/run.json`](/temporal-research-agent/demo/out/run-artifacts/run.json).
...
# live MCP and writer path on the configured workstation
uv run phasee-start --scope full --mode live --artifacts artifacts-live
```
...
| Concurrency | Two episode branches run at a time; stages inside one episode remain sequential. | The sample protects local MCP and writer capacity. A deployed Worker fleet could raise the limit or route stages to separate task queues. |
...
## Separate durable-research skill

This repository also contains
[`run-durable-research`](/temporal-research-agent/skills/run-durable-research/SKILL.md), a
workstation-local agent skill backed by a separate general-purpose
`LiteratureReviewWorkflow`. Its thin client starts, queries, waits for, and
retrieves a cited research report from any agent process configured on this
machine. The skill remains local because its SciX and Code Intelligence
Digest services and indexes are local.

The assignment Workflow remains `PodcastResearchWorkflow`. Keeping the
general skill separate avoids hiding the original podcast business logic
behind a generic four-angle example.
