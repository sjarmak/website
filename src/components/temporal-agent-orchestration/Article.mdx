...
The process pane on the right answers that question with two counters it measures for itself, once a second. `agent procs` counts operating-system processes and reads 2 during each retry window, when the surviving agent and the retry's new process run side by side. `sessions` counts distinct session identities that have written to the work item, and it stays at 1. Two processes over one session is the fix working. The before-state recording on the [companion page](/temporal-agent-orchestration/) shows the failure in the same two counters: there, both read 2.
...
<CastPlayer
  ...
  markers={[
    [3.4, "Arm 1 starts"],
    [8.3, "kill -9: Worker one is gone"],
    [15.8, "Retry re-binds: two processes, one session"],
    [26.1, "Workflow completed"],
    [27.5, "Arm 2 starts"],
    [30.9, "kill -9 before any checkpoint"],
    [38.8, "Retry re-resolves: same session returned"],
    [40.7, "Workflow completed"],
  ]}
  notes={{
    1: "The Worker is dead. The agent is still running. Temporal is waiting out the heartbeat timeout before it retries.",
    2: "The process pane reads 2 because the retry runs alongside the agent that outlived the first Worker. Its sessions counter stays at 1: both processes carry the same session.",
    5: "The second Worker is dead before any checkpoint exists. The retry has to ask the resolver again.",
    6: "Two processes again, sessions still 1: the resolver returned the existing session instead of minting a second one.",
  }}
>
...
In Gas City, Beads owns the durable graph of work and Temporal owns one traversal through that graph. Those are different forms of continuity. The task graph may remain perfectly intact while the procedure moving one task through it disappears. That split follows a fashion already established in the agent bubble: [graph engineering](https://www.langchain.com/blog/3-years-of-graph-engineering-with-langgraph). Temporal ships [the same pairing for LangGraph](https://temporal.io/blog/temporal-langgraph-plugin-durable-execution), and my boundary is narrower than [the stronger version of the claim](https://temporal.io/blog/the-fallacy-of-the-graph-why-your-next-workflow-should-be-code-not-a-diagram), which says durable code should replace the diagram entirely.
