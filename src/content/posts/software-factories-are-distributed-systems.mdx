...
Vercel is also a useful comparison because the thing its factory produces is not the factory itself. Its agents work on the AI SDK, including bug fixes, features, documentation, and backports, while a human retains the merge boundary. Four weeks after launch, Vercel reported that factory agents were authoring 25 to 35 percent of the pull requests merged each week. My incidents below come from a self-hosting system because that is the one I can instrument, break, and inspect end to end, but the coordination boundary is not unique to self-hosting.
