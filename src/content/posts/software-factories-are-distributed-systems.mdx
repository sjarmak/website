...
date: 2026-08-16
description: "What months of running coding agents as a fleet taught me about the machinery around them: work identity, expiring authority, effect contracts, narrow publication, reconciliation, and capacity policy, taught through the system where each one broke."
...
Current systems have arrived at similar decompositions from different directions. OpenAI's [Symphony orchestration](https://openai.com/index/open-source-codex-orchestration-symphony/), Cloudflare's [issue-triage factory](https://blog.cloudflare.com/astro-issue-triage/), and Vercel's [factory for the AI SDK repository](https://vercel.com/blog/building-a-software-factory-for-ai-sdk) all separate some version of durable work state, scheduling, disposable workers, and gated publication. Vercel records runs as success, flawed, blocked, or manual, and only success ships.

Vercel is also a useful comparison point. Its agents work on the AI SDK itself, including bug fixes, features, documentation, and backports, while a human retains the merge boundary, and four weeks after launch Vercel reported that factory agents were authoring 25 to 35 percent of the pull requests merged each week. My own factory runs the same orchestration substrate across evaluation infrastructure like CodeScaleBench, research tooling, developer tools, migrations, this website, and Gas City itself. Gas City shows up disproportionately in the incidents below because it's the one place where I control both the orchestration layer and the code being changed, which makes it unusually good for fault injection, recovery experiments, and inspecting a failure end to end. The detailed incidents here are sampled from an environment where my observability and experimental control are unusually strong, not because the factory exists to maintain Gas City.

These distinctions seem like they should be relatively straightforward, until you operate many agents long enough to discover how many places in the factory can collapse "the attempt stopped" into "the work is done."

Most of this essay is about those places. The short version of what I learned, before I get into the details:

1. Work needs an identity that outlives whatever process happens to be working on it right now.
2. Authority has to expire, and it has to get checked where the write actually lands, not where the writer happens to be at the moment of writing.
3. Anything crossing out of your system needs its own identity, and it needs a real `unknown` outcome you're willing to keep.
4. Verify one exact commit, publish through one small conditional write, and record the link between work and code somewhere the worker can't go back and edit later.
5. Events make it fast, queries make it true, and you need both of them.
6. None of this shows up on a dashboard that only measures activity.

Everything below is how I found out about each of these, by watching my factory break.
...
Mixing them up is where things break. A running agent doesn't prove its bead is still assigned to it. A completed formula doesn't prove its branch ever landed. A closed bead doesn't prove the result went anywhere at all.
...
In Gas City this interval is where a mid-dispatch crash does its damage. A dispatcher that dies mid-route leaves the bead with its routing never stamped, which poisons the claim, and the next cycle looks healthy because nothing recorded that an attempt was in flight.
...
Answering that needs a queryable map of your code: which repositories depend on an API, which in-flight work touches callers of a function some other task is changing, whether a plan was built against a revision that has since moved. This is what code intelligence platforms like [Sourcegraph](https://sourcegraph.com) already do for humans, pointed at a different consumer: the scheduler, not the developer or individual worker agent.

A model's context window can't be that map, and neither can an index that doesn't track revisions. When I fed models context drawn from an outdated revision, 15 of 17 outputs from one model and 13 of 17 from another went straight at an interface that no longer existed. Same setup, retrieval from the current revision, zero incompatible outputs.
...
And you should use the smallest mechanism that actually enforces the invariant. If one human already owns publication, keeping that human in the loop may be the right fence. If your entire failure is "a verified branch never merged," required checks plus auto-merge may solve it without an ownership protocol. If Git's expected-old-SHA conditional update is enough to protect the state you care about, use it. Add an ownership epoch when authority can move while an old worker remains capable of writing. Add durable execution when the procedure itself has state worth recovering. Add reconciliation when truth can be reconstructed more cheaply from current state.
