...
The first real-world subject was Sourcegraph's Deep Search tool, run across eight scenarios: five fully accomplished, one partial, two failed, with polling friction in seven of the eight and an overall usability read of 3.4 out of 5 across comprehension, confidence, friction, composition, and trust calibration. The standing risk the design carries deliberately is cassette rot, since recorded traces drift as upstream tools ship description changes; a freshness check in CI and manual re-recording are the current answer, with no auto-refresh policy yet.
