...
Throughput is a deliberate tradeoff: cloud sandboxes run a batch of a couple hundred tasks in about fifteen minutes of wall clock, local Docker is free and takes hours, and pre-built base images cut per-task builds from minutes to seconds. The repo itself is dual-remote — a private main holding raw traces, and a sanitized public mirror fed through a fail-closed export that re-scans every published file for secrets before anything ships. The open questions that matter most right now are editorial rather than engineering: which task count is canonical, and whether heuristically scored results belong in headline aggregates.
