...
The bones are good: the memory tiers, the preference math, and the test discipline survived the audit intact. The repair list is short and ordered: fix the stdin contract, then make the session-end hook actually invoke the cheap model the routing scaffolding already anticipates, roughly two focused days that convert a convincing scaffold into a testable hypothesis. Until then the project stands as a clean case study in the failure mode where semantic judgments ship as keyword lists wearing an agent-shaped costume, built around the hard part instead of through it.
