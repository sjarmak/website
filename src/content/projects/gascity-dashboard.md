...
The recurring lessons generalized. Two readers of one truth always drift, and the durable fix is structural unification, not synchronized parallel code. Validation at the wrong layer breaks evolution: browser-side strict response validation of the supervisor's open-ended surface caused a production outage and was reverted. Timeouts must be sized to the measured slow path, not the happy path. The repo's terminal goal is to replace `gc dashboard` inside Gas City itself, and every supervisor API gap it papers over client-side is tagged with a deletion condition for that fold-back.
