...
<h2 id="chapter-1">Chapter 1: Run-to-run variance, statistical power, and paired comparisons</h2>
...
### <a id="never-report-a-single-run" href="#never-report-a-single-run">Compare distributions across randomized repeated runs</a>
...
### <a id="power-analyze-before-running" href="#power-analyze-before-running">Power-analyze the evaluation before running it</a>
...
### <a id="use-paired-tests-matched-to-metric" href="#use-paired-tests-matched-to-metric">Compare systems with metric-appropriate paired tests</a>
...
### <a id="report-correlation-aware-standard-errors" href="#report-correlation-aware-standard-errors">Report correlation-aware standard errors with every evaluation score</a>
...
### <a id="estimate-pass-at-k-unbiased" href="#estimate-pass-at-k-unbiased">Estimate pass@k with the unbiased combinatorial estimator</a>
...
In a code-generation evaluation study, Chen and colleagues ([2021](https://arxiv.org/abs/2107.03374)) introduced this combinatorial estimator and showed that it is exactly unbiased for the probability that at least one of <span class="katex"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mi>k</mi></mrow><annotation encoding="application/x-tex">k</annotation></semantics></math></span> draws succeeds, while the plug-in alternative is biased high. Exact unbiasedness does not make an estimate precise: small <span class="katex"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mi>n</mi></mrow><annotation encoding="application/x-tex">n</annotation></semantics></math></span> still leaves substantial per-problem variability. Pass@k also measures sampled coverage under an oracle that can recognize a correct candidate. A deployed system cannot realize that coverage unless it has a sufficiently reliable verifier or selector. The estimator transfers to theorem proving, tool use, and other retry-based tasks only when every sampled candidate can be assessed for correctness.

### <a id="correct-multi-benchmark-victory-claims" href="#correct-multi-benchmark-victory-claims">Correct suite-wide victory claims with replicability analysis</a>
...
### <a id="rank-with-paired-comparison-models-and-cis" href="#rank-with-paired-comparison-models-and-cis">Rank systems with a paired-comparison model, confidence intervals, and active sampling</a>
...
### <a id="profile-benchmark-noise-before-decisions" href="#profile-benchmark-noise-before-decisions">Profile benchmark noise before making decisions and engineer its signal-to-noise ratio upward</a>
...
### <a id="declare-decoding-configuration" href="#declare-decoding-configuration">Declare decoding settings and compare greedy with sampled generation</a>
...
### <a id="aggregate-few-runs-with-iqm-and-bootstrap-cis" href="#aggregate-few-runs-with-iqm-and-bootstrap-cis">Aggregate a handful of runs with IQM, bootstrap intervals, and performance profiles</a>
...
### <a id="shrink-evals-with-decision-aware-validation" href="#shrink-evals-with-decision-aware-validation">Validate every benchmark reduction by its effect on decisions</a>
...
### <a id="precommit-confirmatory-eval-designs" href="#precommit-confirmatory-eval-designs">Pilot the evaluation, pre-commit its design, and confirm once</a>
...
### <a id="report-spread-across-prompt-variants" href="#report-spread-across-prompt-variants">Measure performance across plausible prompt variants</a>
...
<h2 id="chapter-2">Chapter 2: Baselines, ablations, and cost-accuracy tradeoffs</h2>
...
### <a id="run-ablation-controls" href="#run-ablation-controls">Run controls that isolate each component’s contribution</a>
...
### <a id="report-cost-accuracy-pareto" href="#report-cost-accuracy-pareto">Evaluate accuracy and cost on a Pareto frontier</a>
...
### <a id="report-performance-vs-tuning-budget" href="#report-performance-vs-tuning-budget">Report expected best performance across tuning budgets</a>
...
### <a id="budget-sampling-by-coverage-and-verifier" href="#budget-sampling-by-coverage-and-verifier">Budget repeated sampling from coverage and verifier limits</a>
...
### <a id="freeze-then-evaluate-memory" href="#freeze-then-evaluate-memory">Freeze memory before evaluating deployment performance</a>
...
### <a id="pin-the-scoring-target" href="#pin-the-scoring-target">Pin the stored form that receives retrieval credit</a>
...
### <a id="controlled-synthesis-human-verified" href="#controlled-synthesis-human-verified">Synthesize constrained eval instances and require human review</a>
...
### <a id="baseline-gates-against-random" href="#baseline-gates-against-random">Benchmark every patch gate against random selection</a>
...
<h2 id="chapter-3">Chapter 3: Benchmark contamination, oracle strength, and workload validity</h2>
...
### <a id="measure-public-score-inflation-with-matched-controls" href="#measure-public-score-inflation-with-matched-controls">Measure public benchmark inflation with matched private controls</a>
...
### <a id="evaluate-on-post-cutoff-tasks" href="#evaluate-on-post-cutoff-tasks">Evaluate models on post-cutoff tasks</a>
...
### <a id="strengthen-test-oracles-before-adjudication" href="#strengthen-test-oracles-before-adjudication">Strengthen test oracles before adjudicating results</a>
...
### <a id="benchmark-on-your-own-workload" href="#benchmark-on-your-own-workload">Benchmark systems on the workload they will serve</a>
...
### <a id="probe-memorization-before-crediting-capability" href="#probe-memorization-before-crediting-capability">Probe memorization before crediting benchmark gains to capability</a>
...
### <a id="measure-overlap-semantically" href="#measure-overlap-semantically">Measure overlap with semantic matching</a>
...
### <a id="anchor-contamination-metrics-to-score-benefit" href="#anchor-contamination-metrics-to-score-benefit">Investigate contamination through estimated score benefit</a>
...
### <a id="build-fresh-items-anti-searchable" href="#build-fresh-items-anti-searchable">Build fresh items under an anti-searchability protocol</a>
...
### <a id="audit-contamination-per-model-benchmark-cell" href="#audit-contamination-per-model-benchmark-cell">Audit each model-benchmark cell separately</a>
...
### <a id="combine-contamination-detectors-by-threat-model" href="#combine-contamination-detectors-by-threat-model">Combine contamination detectors by threat model</a>
...
### <a id="run-construct-validity-check" href="#run-construct-validity-check">Run a construct-validity check before adopting or building a benchmark</a>
...
### <a id="build-code-benchmarks-against-defect-taxonomy" href="#build-code-benchmarks-against-defect-taxonomy">Investigate code-benchmark design against the documented defect taxonomy</a>
...
### <a id="give-agent-benchmarks-real-holdouts" href="#give-agent-benchmarks-real-holdouts">Split agent holdouts by the claimed level of generalization</a>
...
### <a id="pin-and-publish-eval-harness" href="#pin-and-publish-eval-harness">Pin the eval harness and publish its complete artifacts</a>
...
### <a id="stress-test-rankings-under-perturbation" href="#stress-test-rankings-under-perturbation">Stress-test rankings under task and harness perturbations</a>
...
### <a id="protect-eval-data-at-release" href="#protect-eval-data-at-release">Protect evaluation data when releasing it</a>
...
### <a id="purge-invalid-items-before-measuring" href="#purge-invalid-items-before-measuring">Purge invalid items before measuring reliability</a>
...
### <a id="audit-leaderboard-submission-protocols" href="#audit-leaderboard-submission-protocols">Audit the leaderboard submission protocol before trusting its rankings</a>
...
### <a id="audit-pipeline-against-qrp-catalog" href="#audit-pipeline-against-qrp-catalog">Treat a questionable-practices catalog as an investigative lead</a>
...
### <a id="search-before-building-a-benchmark" href="#search-before-building-a-benchmark">Search for an adequate benchmark before creating another</a>
...
### <a id="pair-offline-and-online-evals" href="#pair-offline-and-online-evals">Pair offline evaluation with online experiments and recalibrate the judge</a>
...
### <a id="checklist-agentic-benchmarks-for-validity" href="#checklist-agentic-benchmarks-for-validity">Check task validity and outcome validity before using an agent benchmark</a>
...
<h2 id="chapter-4">Chapter 4: Execution-based evaluation, correction gates, and release tests</h2>
...
### <a id="ground-evaluation-in-execution" href="#ground-evaluation-in-execution">Gate candidate work on execution results</a>
...
### <a id="gate-self-correction-on-external-feedback" href="#gate-self-correction-on-external-feedback">Require external feedback for every correction step</a>
...
### <a id="golden-set-pass-k" href="#golden-set-pass-k">Replay a golden task set and score repeated success</a>
...
### <a id="score-verified-resolve-with-abstention" href="#score-verified-resolve-with-abstention">Score verified resolution separately and reward abstention</a>
...
### <a id="detect-silent-tool-errors" href="#detect-silent-tool-errors">Detect silent tool errors before planning</a>
...
### <a id="cap-turn-budgets" href="#cap-turn-budgets">Cap agent turns and extend budgets only on evidence of progress</a>
...
### <a id="budget-the-verifier-tax" href="#budget-the-verifier-tax">Budget enforcement overhead and test recovery after blocks</a>
...
<h2 id="chapter-5">Chapter 5: Calibrating model graders and separating agreement from correctness</h2>
...
### <a id="calibrate-llm-judges" href="#calibrate-llm-judges">Calibrate model judges against expert labels</a>
...
### <a id="separate-agreement-from-correctness" href="#separate-agreement-from-correctness">Keep agreement separate from correctness</a>
...
### <a id="keep-an-exact-auditor-alongside-heuristics" href="#keep-an-exact-auditor-alongside-heuristics">Keep an exact auditor beside heuristic graders</a>
...
### <a id="combine-judge-labels-with-human-subset" href="#combine-judge-labels-with-human-subset">Combine judge labels with a random human-labeled subset</a>
...
<h2 id="chapter-6">Chapter 6: Proxy metric gaming and layered evaluation signals</h2>
...
### <a id="layer-signals-beyond-single-proxy" href="#layer-signals-beyond-single-proxy">Layer independent signals into every acceptance gate</a>
...
### <a id="monitor-proxy-true-divergence" href="#monitor-proxy-true-divergence">Monitor divergence between proxy and true signals</a>
...
### <a id="monitor-cot-without-training-pressure" href="#monitor-cot-without-training-pressure">Monitor chain of thought without optimizing against it</a>
...
### <a id="escalate-on-first-gaming" href="#escalate-on-first-gaming">Escalate oversight at the first sign of gaming</a>
...
### <a id="harden-infeasible-tasks" href="#harden-infeasible-tasks">Harden the environment around infeasible tasks</a>
...
### <a id="measure-reasoning-effort" href="#measure-reasoning-effort">Measure how early reward appears in the reasoning</a>
...
### <a id="weight-average-reward-models" href="#weight-average-reward-models">Average compatible reward models in weight space</a>
...
### <a id="evaluate-in-feedback-loops" href="#evaluate-in-feedback-loops">Evaluate agents inside their deployment feedback loops</a>
...
### <a id="test-detectors-contrastively" href="#test-detectors-contrastively">Test hack detectors on realistic contrastive mixtures</a>
...
### <a id="accept-on-vector-with-independent-judge" href="#accept-on-vector-with-independent-judge">Explore vector acceptance with an independent judge</a>
...
<h2 id="chapter-7">Chapter 7: The software factory as a distributed system</h2>
...
<h2 id="chapter-8">Chapter 8: Agent isolation, injection defenses, and independent verification</h2>
...
### <a id="contain-agent-blast-radius" href="#contain-agent-blast-radius">Contain each agent’s blast radius</a>
...
### <a id="distrust-agent-self-reports" href="#distrust-agent-self-reports">Verify every agent completion claim</a>
...
### <a id="defense-in-depth-injection" href="#defense-in-depth-injection">Defend against prompt injection in depth</a>
...
### <a id="topology-as-security-decision" href="#topology-as-security-decision">Make orchestration topology part of the security design</a>
...
### <a id="stateful-cross-session-guardrails" href="#stateful-cross-session-guardrails">Investigate stateful guardrails across sessions</a>
...
### <a id="execute-agent-remediations-through-typed-policy-path" href="#execute-agent-remediations-through-typed-policy-path">Route agent remediations through a typed policy path</a>
...
### <a id="score-actions-by-reversibility-and-blast-radius" href="#score-actions-by-reversibility-and-blast-radius">Assign autonomy separately to every action</a>
...
### <a id="measure-your-own-security-delta" href="#measure-your-own-security-delta">Investigate the local security effect of AI assistance</a>
...
### <a id="never-accept-self-attestation" href="#never-accept-self-attestation">Reject self-attestation for AI-assisted code</a>
...
<h2 id="chapter-9">Chapter 9: Persistent agent state, durable workflows, and idempotent retries</h2>
...
### <a id="make-agent-state-first-class-persistent" href="#make-agent-state-first-class-persistent">Make agent state a persistent artifact</a>
...
### <a id="use-durable-workflow-engine" href="#use-durable-workflow-engine">Coordinate multi-step work with a durable workflow engine</a>
...
### <a id="make-retried-steps-idempotent" href="#make-retried-steps-idempotent">Make every retried step idempotent</a>
...
### <a id="transactional-side-effects" href="#transactional-side-effects">Give every external effect a receipt and a compensator</a>
...
### <a id="state-retry-contract-invariants" href="#state-retry-contract-invariants">Define the runtime retry contract with three observable invariants</a>
...
### <a id="checkpoint-async-at-consistency-boundaries" href="#checkpoint-async-at-consistency-boundaries">Take asynchronous snapshots at natural consistency boundaries</a>
...
### <a id="calibrate-checkpoint-interval" href="#calibrate-checkpoint-interval">Calculate checkpoint cadence from measured operating conditions</a>
...
### <a id="design-event-coordination-guarantees-explicitly" href="#design-event-coordination-guarantees-explicitly">Investigate event-delivery guarantees before choosing choreography</a>
...
### <a id="log-steps-transactionally-with-data" href="#log-steps-transactionally-with-data">Commit each step’s execution record with its data changes</a>
...
### <a id="isolate-shared-agent-state-transactionally" href="#isolate-shared-agent-state-transactionally">Isolate shared agent state with transactional controls</a>
...
### <a id="guard-stochastic-steps-with-postconditions" href="#guard-stochastic-steps-with-postconditions">Investigate deterministic postconditions for stochastic steps</a>
...
### <a id="version-events-weak-schema-first" href="#version-events-weak-schema-first">Version events early and strengthen their schema gradually</a>
...
### <a id="design-operations-order-tolerant" href="#design-operations-order-tolerant">Design operations to tolerate partial execution order</a>
...
### <a id="crystallize-repeated-plans-into-blueprints" href="#crystallize-repeated-plans-into-blueprints">Investigate crystallizing repeated plans into declarative blueprints</a>
...
### <a id="separate-logical-work-from-attempt-identity" href="#separate-logical-work-from-attempt-identity">Separate logical work identity from attempt identity</a>
...
### <a id="reconcile-unknown-external-effects" href="#reconcile-unknown-external-effects">Reconcile unknown external effects instead of assuming</a>
...
### <a id="version-verifier-results" href="#version-verifier-results">Version verifier results</a>
...
<h2 id="chapter-10">Chapter 10: Replayable traces and fault-injection recovery testing</h2>
...
### <a id="replayable-typed-traces" href="#replayable-typed-traces">Record every run as a replayable typed trace</a>
...
### <a id="benchmark-recovery-with-fault-injection" href="#benchmark-recovery-with-fault-injection">Benchmark recovery by injecting failures</a>
...
### <a id="record-nondeterminism-replay-deterministic-code" href="#record-nondeterminism-replay-deterministic-code">Record nondeterministic results and replay only deterministic code</a>
...
### <a id="bound-replay-history" href="#bound-replay-history">Bound the event history that recovery must replay</a>
...
### <a id="provenance-driven-reexecution" href="#provenance-driven-reexecution">Recover heterogeneous workflows from task-level provenance</a>
...
### <a id="certify-restores-against-downstream-commitments" href="#certify-restores-against-downstream-commitments">Certify restore points against committed downstream work</a>
...
### <a id="diagnose-then-gate-recovery" href="#diagnose-then-gate-recovery">Diagnose failures before admitting guidance to a retry</a>
...
### <a id="hide-recovery-behind-monotonic-observability" href="#hide-recovery-behind-monotonic-observability">Expose recovery as failure-free only through monotonic observations</a>
...
### <a id="fault-inject-with-realistic-fault-menu" href="#fault-inject-with-realistic-fault-menu">Inject semantic failures from a realistic fault menu</a>
...
### <a id="test-controller-under-stale-and-missed-state" href="#test-controller-under-stale-and-missed-state">Test the controller under stale and missed state</a>
...
### <a id="place-faults-at-named-commit-barriers" href="#place-faults-at-named-commit-barriers">Place fault injections at named commit barriers</a>
...
### <a id="test-the-recovery-action" href="#test-the-recovery-action">Test the recovery action itself</a>
...
### <a id="measure-recovery-under-live-load" href="#measure-recovery-under-live-load">Measure recovery under live load</a>
...
<h2 id="chapter-11">Chapter 11: Human-auditable failure analysis and taxonomy development</h2>
...
### <a id="derive-taxonomy-from-own-traces" href="#derive-taxonomy-from-own-traces">Derive the failure taxonomy from your own traces</a>
...
### <a id="keep-humans-in-failure-attribution" href="#keep-humans-in-failure-attribution">Keep humans responsible for failure attribution</a>
...
### <a id="design-traces-for-human-audit" href="#design-traces-for-human-audit">Design traces for human audit</a>
...
### <a id="judge-from-constraint-violation-log" href="#judge-from-constraint-violation-log">Localize failures from a constraint-violation log</a>
...
### <a id="monitor-early-trajectory-shape" href="#monitor-early-trajectory-shape">Monitor early trajectory shape during execution</a>
...
### <a id="preempt-environment-errors" href="#preempt-environment-errors">Preempt environment errors before reasoning begins</a>
...
### <a id="aggregate-independent-attribution-judges" href="#aggregate-independent-attribution-judges">If automating attribution, investigate hierarchical context and independent judges</a>
...
### <a id="differential-test-before-blaming-agent" href="#differential-test-before-blaming-agent">Differentially test the agent against an environment oracle</a>
...
### <a id="canonicalize-trajectories" href="#canonicalize-trajectories">Canonicalize trajectories without discarding source events</a>
...
### <a id="use-run-consistency-as-risk-signal" href="#use-run-consistency-as-risk-signal">Use run-to-run consistency as a per-task risk signal</a>
...
<h2 id="chapter-12">Chapter 12: Measuring and designing repository retrieval</h2>
...
### <a id="score-retrieval-and-generation-separately" href="#score-retrieval-and-generation-separately">Score retrieval separately from final answers</a>
...
### <a id="hybrid-retrieval-fused-on-ranks" href="#hybrid-retrieval-fused-on-ranks">Run hybrid retrieval and fuse the ranked results</a>
...
### <a id="chunk-on-ast-boundaries" href="#chunk-on-ast-boundaries">Chunk code along AST boundaries</a>
...
### <a id="retrieve-across-lexical-gaps" href="#retrieve-across-lexical-gaps">Bridge lexical gaps before generation</a>
...
### <a id="iterate-retrieval-with-generation" href="#iterate-retrieval-with-generation">Re-query the repository as the draft develops</a>
...
### <a id="gate-retrieval-on-expected-benefit" href="#gate-retrieval-on-expected-benefit">Gate retrieval on its expected downstream benefit</a>
...
### <a id="fix-both-ends-of-rag" href="#fix-both-ends-of-rag">Test retrieval quality and context use independently</a>
...
### <a id="stage-cheap-retrieval-then-rerank" href="#stage-cheap-retrieval-then-rerank">Prune cheaply, then rerank a small candidate set</a>
...
<h2 id="chapter-13">Chapter 13: Localization funnels, repository indexes, and freshness checks</h2>
...
### <a id="staged-localization-funnel" href="#staged-localization-funnel">Localize repository changes through a staged funnel</a>
...
### <a id="index-repository-as-knowledge-graph" href="#index-repository-as-knowledge-graph">Index the repository as a typed knowledge graph</a>
...
### <a id="gate-retrieval-on-freshness" href="#gate-retrieval-on-freshness">Gate retrieved context on freshness and authorization</a>
...
### <a id="retrieve-by-structure" href="#retrieve-by-structure">Retrieve completion context through structural relations</a>
...
### <a id="front-load-a-repo-map" href="#front-load-a-repo-map">Front-load a condensed repository map</a>
...
### <a id="inject-static-anchors" href="#inject-static-anchors">Inject static-analysis facts beside the code</a>
...
### <a id="schedule-exploration-prune-by-distance" href="#schedule-exploration-prune-by-distance">Schedule exploration by relevance and prune it by graph distance</a>
...
### <a id="walk-call-graph-at-query-time" href="#walk-call-graph-at-query-time">Walk the current call graph when no maintained index is available</a>
...
### <a id="extend-graph-below-function-level-for-line-precision" href="#extend-graph-below-function-level-for-line-precision">Add statement-level def-use edges for line precision</a>
...
### <a id="route-entity-queries-to-cheap-graph-lookups" href="#route-entity-queries-to-cheap-graph-lookups">Route entity queries to cheap graph lookups and reserve model exploration for ambiguous questions</a>
...
### <a id="treat-index-representation-as-first-class-design-choice" href="#treat-index-representation-as-first-class-design-choice">Compare indexed representations before choosing one</a>
...
<h2 id="chapter-14">Chapter 14: Usable context budgets, consolidated-spec restarts, and file-based tool output</h2>
...
### <a id="budget-to-measured-effective-context" href="#budget-to-measured-effective-context">Budget context using measured effective capacity</a>
...
### <a id="consolidate-spec-restart-lost-runs" href="#consolidate-spec-restart-lost-runs">Consolidate the specification and restart lost runs</a>
...
### <a id="persist-transient-context-as-files" href="#persist-transient-context-as-files">Persist large transient outputs as files</a>
...
### <a id="measure-context-files-and-maintain-them-like-config" href="#measure-context-files-and-maintain-them-like-config">Evaluate and maintain context files like configuration</a>
...
### <a id="edge-load-key-evidence" href="#edge-load-key-evidence">Put load-bearing evidence at the edges of the context</a>
...
### <a id="verify-semantic-not-verbatim-recall" href="#verify-semantic-not-verbatim-recall">Qualify context with semantic tracing instead of needle recall</a>
...
### <a id="compaction-as-agent-action" href="#compaction-as-agent-action">Give the agent a milestone-triggered compaction action</a>
...
### <a id="price-token-reduction-transforms" href="#price-token-reduction-transforms">Measure the task cost of every context-reduction transform</a>
...
### <a id="keep-always-loaded-context-minimal" href="#keep-always-loaded-context-minimal">Keep permanent context small and verify deferred capabilities</a>
...
### <a id="audit-the-assembled-context-budget" href="#audit-the-assembled-context-budget">Budget every contributor and inspect the assembled context</a>
...
<h2 id="chapter-15">Chapter 15: Cross-session memory, raw traces, and compaction policies</h2>
...
### <a id="retain-raw-distill-separately" href="#retain-raw-distill-separately">Retain raw traces and distill them separately</a>
...
### <a id="light-store-by-default" href="#light-store-by-default">Start cross-session memory with a lightweight transcript store</a>
...
### <a id="optimize-compaction-from-failures" href="#optimize-compaction-from-failures">Optimize compaction rules from observed failures</a>
...
### <a id="persist-explored-context" href="#persist-explored-context">Persist explored repository context between reasoning steps</a>
...
### <a id="durable-artifact-handoff" href="#durable-artifact-handoff">Investigate durable references for agent handoffs</a>
...
### <a id="scheduled-typed-consolidation" href="#scheduled-typed-consolidation">Consolidate memory on a schedule and govern the resulting store</a>
...
### <a id="model-time-explicitly" href="#model-time-explicitly">Define whether each fact is superseded or versioned</a>
...
### <a id="plan-for-memory-portability" href="#plan-for-memory-portability">Investigate an exportable contract for agent memory</a>
...
### <a id="tribal-knowledge-substrate" href="#tribal-knowledge-substrate">Maintain a retrieval substrate for knowledge outside the repository</a>
...
<h2 id="chapter-16">Chapter 16: Efficient verification interfaces and risk-based human escalation</h2>
...
### <a id="make-verification-cheaper-than-acceptance" href="#make-verification-cheaper-than-acceptance">Make Verification Cheaper Than Blind Acceptance</a>
...
### <a id="target-friction-at-the-accept-loop" href="#target-friction-at-the-accept-loop">Put Friction at the Moment of Acceptance</a>
...
### <a id="escalate-only-flagged-cases" href="#escalate-only-flagged-cases">Escalate Flagged Cases to Human Verdict Owners</a>
...
### <a id="place-human-checkpoints-at-failure-points" href="#place-human-checkpoints-at-failure-points">Place human checkpoints at measured failure points with an owned escalation route</a>
...
### <a id="design-overseer-roles-for-skill-retention" href="#design-overseer-roles-for-skill-retention">Design overseer roles to preserve intervention skill</a>
...
### <a id="calibrate-displayed-confidence" href="#calibrate-displayed-confidence">Calibrate confidence signals before showing them to reviewers</a>
...
### <a id="curate-what-reviewers-see" href="#curate-what-reviewers-see">Curate displayed suggestions and highlight edit-likely tokens</a>
...
### <a id="shield-review-from-time-pressure" href="#shield-review-from-time-pressure">Shield high-stakes review from deadline pressure</a>
...
### <a id="render-agent-actions-as-live-state" href="#render-agent-actions-as-live-state">Investigate whether live step-level state could support mid-flight steering</a>
...
### <a id="treat-human-escalation-as-a-budgeted-online-decision" href="#treat-human-escalation-as-a-budgeted-online-decision">Allocate scarce human review with an adaptive budgeted deferral policy</a>
...
### <a id="budget-verification-as-the-dominant-cost" href="#budget-verification-as-the-dominant-cost">Investigate whether verification dominates AI-assisted coding costs</a>
...
<h2 id="chapter-17">Chapter 17: Autonomy calibration, provenance, effective gates, and accountability</h2>
...
### <a id="graduate-autonomy-per-action-track-record" href="#graduate-autonomy-per-action-track-record">Graduate Each Action’s Autonomy from Its Track Record</a>
...
### <a id="label-ai-provenance" href="#label-ai-provenance">Label AI-Generated Code with Its Provenance</a>
...
### <a id="audit-human-gates-for-effectiveness" href="#audit-human-gates-for-effectiveness">Audit Human Gates for Real Effectiveness</a>
...
### <a id="align-accountability-with-actual-control" href="#align-accountability-with-actual-control">Align Accountability with Actual Human Control</a>
...
### <a id="attach-reliance-disclaimers" href="#attach-reliance-disclaimers">Test plain reliance disclaimers before stronger interventions</a>
...
### <a id="measure-oversight-with-decomposed-metrics" href="#measure-oversight-with-decomposed-metrics">Decompose oversight metrics before reporting coverage</a>
...
### <a id="scale-review-capacity-to-experience" href="#scale-review-capacity-to-experience">Allocate review capacity by contributor experience</a>
...
### <a id="record-steps-in-hash-chained-ledger" href="#record-steps-in-hash-chained-ledger">Record workflow steps in a hash-chained ledger</a>
...
### <a id="rebuild-collective-accountability" href="#rebuild-collective-accountability">Restore collective accountability around agent-reviewed work</a>
...
### <a id="separate-control-from-oversight" href="#separate-control-from-oversight">Investigate control and oversight as separate supervision functions</a>
...
### <a id="require-comprehension-before-merge" href="#require-comprehension-before-merge">Test comprehension before merging generated code</a>
...
### <a id="write-an-oversight-card" href="#write-an-oversight-card">Investigate an oversight card for each deployed system</a>
...
### <a id="keep-override-authority-real" href="#keep-override-authority-real">Investigate whether experts retain usable override authority</a>
...
### <a id="write-an-agent-contribution-policy" href="#write-an-agent-contribution-policy">Define six dimensions of agent-contribution policy</a>
...
### <a id="set-autonomy-defaults-per-task-type" href="#set-autonomy-defaults-per-task-type">Set autonomy defaults by task type</a>
...
### <a id="treat-objective-weights-as-reviewed-policy" href="#treat-objective-weights-as-reviewed-policy">Review objective terms and weights as policy</a>
...
### <a id="gate-optimizer-output-behind-human-review" href="#gate-optimizer-output-behind-human-review">Gate optimizer output behind developer review</a>
...
<h2 id="chapter-18">Chapter 18: Agent topology selection and dynamic task allocation</h2>
...
### <a id="fix-failures-structurally-not-prompts" href="#fix-failures-structurally-not-prompts">Fix recurring failure classes through system structure</a>
...
### <a id="single-agent-baseline-gate" href="#single-agent-baseline-gate">Gate multi-agent fan-out with a single-agent baseline</a>
...
### <a id="match-topology-to-task-shape" href="#match-topology-to-task-shape">Match the orchestration topology to the task</a>
...
### <a id="dynamic-task-graphs" href="#dynamic-task-graphs">Schedule work through dynamic task graphs</a>
...
### <a id="oversee-mas-as-graph" href="#oversee-mas-as-graph">Oversee multi-agent execution at node, edge, and path levels</a>
...
### <a id="roles-in-config-not-code" href="#roles-in-config-not-code">Express agent roles through configuration and prompts</a>
...
### <a id="blind-review-lanes" href="#blind-review-lanes">Blind review lanes and conceal interim tallies</a>
...
### <a id="active-blackboard-with-diversity" href="#active-blackboard-with-diversity">Activate blackboard writes and test a diversity check</a>
...
### <a id="repair-root-cause-not-last-step" href="#repair-root-cause-not-last-step">Repair the earliest supported root cause</a>
...
### <a id="fence-stale-worker-authority" href="#fence-stale-worker-authority">Fence stale worker authority at the mutation boundary</a>
...
### <a id="re-decide-cheaply-ship-the-incumbent" href="#re-decide-cheaply-ship-the-incumbent">Recompute schedules cheaply and ship the incumbent</a>
...
### <a id="route-work-to-the-cheapest-sufficient-model" href="#route-work-to-the-cheapest-sufficient-model">Route each task to the cheapest sufficient model</a>
...
### <a id="replay-traces-before-policy-changes" href="#replay-traces-before-policy-changes">Replay recorded traces before changing dispatch policy</a>
...
### <a id="bound-admission-and-recovery-traffic" href="#bound-admission-and-recovery-traffic">Bound admitted demand, isolate demand domains, and release recovery traffic at a controlled rate</a>
...
### <a id="formalize-work-as-constraints-and-solve-globally" href="#formalize-work-as-constraints-and-solve-globally">Formalize work as constraints and solve the allocation globally</a>
...
### <a id="design-the-online-rescheduling-loop" href="#design-the-online-rescheduling-loop">Design rescheduling as a closed-loop algorithm</a>
...
### <a id="probe-only-above-an-uncertainty-threshold" href="#probe-only-above-an-uncertainty-threshold">Investigate threshold-gated probing when uncertainty is costly</a>
...
### <a id="split-fleet-scheduling-into-global-and-local-levels" href="#split-fleet-scheduling-into-global-and-local-levels">Split scheduling between global coordination and local workers</a>
...
### <a id="audit-the-allocation-layer" href="#audit-the-allocation-layer">Investigate the allocation layer as a shared audited component</a>
...
### <a id="cost-cascade-routing" href="#cost-cascade-routing">Route cheap-first and escalate through an uncertainty gate</a>
...
### <a id="log-scheduling-decisions-for-selection-effect-modeling" href="#log-scheduling-decisions-for-selection-effect-modeling">Investigate decision logging for later selection-effect modeling</a>
...
### <a id="pilot-to-pick-the-algorithm-then-commit-the-budget" href="#pilot-to-pick-the-algorithm-then-commit-the-budget">Pick the algorithm with short pilots, then commit the long budget</a>
...
### <a id="budget-retries-as-load" href="#budget-retries-as-load">Budget retries as load</a>
...
### <a id="jitter-recovery-redelivery" href="#jitter-recovery-redelivery">Jitter recovery redelivery</a>
...
### <a id="reserve-or-prioritize-recovery-capacity" href="#reserve-or-prioritize-recovery-capacity">Reserve or prioritize capacity for recovery work</a>
...
### <a id="separate-global-capacity-from-local-dispatch" href="#separate-global-capacity-from-local-dispatch">Separate global capacity from local dispatch</a>
...
### <a id="detect-retry-storms-and-recovery-starvation" href="#detect-retry-storms-and-recovery-starvation">Detect retry storms and recovery starvation</a>
