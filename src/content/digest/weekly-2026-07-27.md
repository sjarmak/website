...
A [Databricks benchmark on a multi-million-line, multi-language codebase](https://www.reddit.com/r/LLMDevs/comments/1v6c83s/are_we_measuring_codingagent_cost_at_the_wrong/) found that token price predicts almost nothing about the cost of finishing an engineering task. Same model, different harness, more than twice the cost. One harness sent roughly 3x less context per turn and reached similar quality. The thread's proposed metric is cost per verified task: model plus tool plus context plus retry cost, divided by a task that passes its checks. The open question it raises is the one worth arguing about, whether you hold the harness constant when comparing agents or treat the harness as part of what you're evaluating. The Leni decomposition and the terminal-review cost spread both say the second.
