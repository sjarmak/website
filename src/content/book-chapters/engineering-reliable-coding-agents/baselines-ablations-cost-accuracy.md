...
The evidence base for this chapter is narrow. It holds five items across two taught entries. Four of them are explorer-class syntheses, and none of those four is a controlled experiment. One of the two entries has no strong evidence item at all. I therefore use the evidence to specify comparisons readers can run on their own workloads. The record cannot support a universal cost threshold or a recommended architecture.
...
The evidence for this practice is explorer-class synthesis, which the evidence audit retained as strong although no single controlled experiment supplies the support. The same control logic appeared independently in two corpora. In agent memory, the spine of that synthesis is SkillEvolBench, from Lei et al. ([2026](https://arxiv.org/abs/2605.24117)). In code retrieval, CoIR, from Li et al. ([2024](https://arxiv.org/abs/2407.02883)), supplies only the task and metric substrate, and the control logic below comes from the explorer's evaluation-design blocks rather than from that benchmark paper. The provenance is imperfect, because the memory explorer named three works behind a single carried identifier before that record was split. Two of the three are now carried at directional strength with no figure of their own. The support rests on convergence between the corpora rather than on one reported effect size.
