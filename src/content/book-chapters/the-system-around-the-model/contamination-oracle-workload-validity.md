...
I treat any public score as the output of three coupled measurements. First, has the model already encountered the work? Second, can the test oracle refute an incorrect answer? Third, does the task distribution represent the work someone wants the model to do? The sponsor's reversal is the demonstration that reputation, curation effort, and a large annotator count answer none of these questions. Each question needs its own control.
...
The first control is a point-in-time audit any reader can commission. A **matched control set** reproduces the public suite's difficulty features on tasks the benchmark never released, and running both sets under the same protocol yields a per-model gap. Prathifkumar et al. ([2025](https://arxiv.org/abs/2512.10218)) ran that comparison observationally and found roughly a 3x overall advantage and a 6x file-localization advantage on SWE-bench Verified relative to control tasks. Their matching covered popular open-source Python projects. Unresolved differences in task difficulty therefore remain a plausible explanation for part of the gap. This audit is the point-in-time measurement, and the post-cutoff pipeline in the next section is the standing control that keeps it from having to be recommissioned as the public set ages.

Training history is usually inaccessible to the evaluator. Model providers rarely publish complete corpora, and a search through a disclosed corpus would still miss later fine-tuning, generated training examples, and indirect exposure through task discussions. Asking whether a model has seen a specific item produces an attribution problem with no reliable ground truth. Asking how its performance changes on comparable unseen work produces an observable difference.

The audit requires two task sets that differ in public exposure and agree as closely as possible on the features that determine difficulty. For repository work, the matching should cover language, repository scale, issue form, expected patch size, dependency burden, and the amount of localization required before editing. The model, harness, prompt, tool access, retry policy, and scoring procedure then stay fixed across both sets. A paired design is preferable when each public task has a defensible counterpart, because the analysis can use per-pair differences.

The estimate is the difference between the two scores for one model:
...
The name needs care. The gap estimates combined public-score inflation and cannot isolate a pure training-data effect. It includes direct exposure to tasks and solutions, familiarity with repositories and issue conventions, and the repeated-selection channel from Chapter 2. Every time developers tune an agent against a public suite, retain a change that raises the score, and discard one that lowers it, they adapt to that suite even when the model weights never change. The iteration holdout from Chapter 2 protects a local development process from that channel. The matched comparison estimates whatever inflation remains from all channels together.

One audit is also one realization. The gap inherits the run-to-run variation described in Chapter 1, so each side needs repeated runs before the difference between them is read as a measurement rather than a draw.

There are three practical ways to obtain the comparison set. A private mirror can reproduce the public suite's collection procedure using tasks that have never been released. A newly collected set can sample recent work from the same task family. A retroactive twin can reconstruct questions with the same measured properties when the model's training cutoff predates the twin's publication. Each option answers a narrower question than the phrase 'clean benchmark' suggests.

A private mirror provides the strongest control over exposure, but it is expensive. Repository tasks need reproducible code states, issue text, reference changes, and tests that discriminate acceptable from unacceptable patches. The mirror loses that protection as soon as it is released, sent broadly to vendors, or used repeatedly for agent development. Access logs and handling rules therefore become part of the measurement, because the set's private status changes over time.

Newly collected tasks cost less when a team already has a flow of resolved work. Recent tasks are less likely to have been exposed, but recency can introduce a different codebase mix, framework generation, and issue-selection process. A lower score may reflect unfamiliar work, harder work, or both. Matching therefore needs an explicit adequacy check. A table of similar averages is insufficient. The check should establish whether the distributions overlap, whether individual pairs make engineering sense, and whether the conclusion holds after visibly poor matches are removed.

Retroactive twins make the matching problem unusually visible. Haimes et al. ([2024](https://arxiv.org/abs/2410.09247)) constructed twins for a public question-answering benchmark and required them to pass four statistical indistinguishability gates before comparison. Across 20 models, some public scores exceeded twin scores by as much as 16 percentage points. A measured public-private gap can therefore be large even after formal matching checks have been satisfied.

Those results do not establish a comparable gap for repository-scale coding. The twins were built and validated on question answering, and the construction is available only for models whose cutoff predates the twin's publication. The 16-percentage-point figure is the largest gap that study measured under its own design, not a bound on what a repository-scale coding comparison would show and not a discount to apply to coding scores.

A controlled arithmetic study supplies a second boundary. Zhang et al. ([2024](https://arxiv.org/abs/2405.00332)) rebuilt a grade-school arithmetic benchmark as a private mirror, measured accuracy drops of up to 8 percent, and found that the model-level gap correlated with the probability that a model would generate public items verbatim, at Spearman r^2 = 0.36. Many frontier systems showed minimal overfitting, and every evaluated model generalized to the novel items. That variation is the whole point of running the audit per model. Exposure and score inflation are properties of a model-benchmark pair, and an average correction applied across models would remove the model-specific signal the audit exists to recover.

The observational coding result is more directly relevant to software work and less controlled. Both sides used popular open-source Python projects, but matching at that level does not establish equivalent issue difficulty. A model may localize files more easily in the small set of repositories it has encountered repeatedly, which is itself a useful familiarity effect. The overall performance difference may also contain repository-specific complexity. The 3x and 6x gaps should be reported as directional evidence, with the matching caveat attached to every use of those figures.

The audit should produce a per-model table with public score, matched score, paired difference where pairing is justified, and uncertainty for each quantity. It should also describe the task-matching procedure closely enough that another evaluator can challenge it. A single pooled gap is inadequate, because models differ in exposure and generalization. A single corrected leaderboard is worse, because it converts uncertain, model-specific estimates into false precision.

The decision that follows depends on the size and stability of the gap. A small gap with a wide confidence interval says the audit lacked resolution. A large gap that survives plausible rematching says the public score is a poor estimate for that model on comparable unseen work. Neither result settles what the model will do on the reader's repositories. It establishes only whether public availability appears to change the measurement.
...
The evidence class and strength on each entry below come from its catalog record. Author-system cases in this chapter are narrative and methodology illustrations, not evidence. They support no empirical claim or capability number.

### Opening scene

- OpenAI, "Why we no longer evaluate SWE-bench Verified," 2026-02-20, https://openai.com/index/why-we-no-longer-evaluate-swe-bench-verified. [practitioner / corpus-verified]
- swyx, "SWE-Bench Verified is dead!!", 2026-02-23, breakdown of the OpenAI audit. [practitioner / corpus-verified] Source of the 16.4% full-set figure, which is derived from the audited sample and does not appear verbatim in the post summary.
- Tejal Patwardhan, public statement on frontier evaluations, 2026-06-16: "saturated or gamed." [practitioner / corpus-verified]
- SWE-bench Verified card: 500 instances, 93 professional screeners. [reference / benchmark card]

### Measure public-score inflation with matched controls

- Prathifkumar, Saji Mathews & Nagappan (2025), arXiv:2512.10218, Waterloo. [lit / directional]
- Zhang, H. et al. (2024), GSM1k, arXiv:2405.00332, Scale AI. [lit / strong]
- Haimes, Wenner et al. (2024), arXiv:2410.09247, Apart Research. [lit / strong]
- Corroboration: none on record.

### Evaluate on post-cutoff tasks

- Jain et al. (2024), arXiv:2403.07974, LiveCodeBench, ICLR 2025. [lit / strong]
- Zhang, L. et al. (2025), SWE-bench Goes Live!, arXiv:2505.23419, Microsoft Research. [lit / directional]
- Adamenko et al. (2025), arXiv:2507.11059, SWE-MERA. [lit / directional]
- Badertdinov et al. (2025), arXiv:2505.20411, SWE-rebench, Nebius. [lit / directional]
- Chen, S. et al. (2025), arXiv:2502.17521, static-to-dynamic benchmark survey. [lit / directional]

### Strengthen test oracles before adjudication

- Liu, Xia, Wang & Zhang (2023), arXiv:2305.01210, HumanEval+ / EvalPlus, NeurIPS 2023. [lit / strong]
- Yu, S. et al. (2025), arXiv:2506.09289, UTBoost. [lit / strong]
- Ye, Martinez & Monperrus (2019), arXiv:1909.13694. [lit / strong]
- Aleithan et al. (2024), arXiv:2410.06992, SWE-Bench+. [lit / directional]
- Yu, B. et al. (2026), arXiv:2603.00520, SWE-ABS. [lit / strong]

### Benchmark on your own workload

- Jha, Paltenghi, Maddila, Murali, Ugare & Chandra (2026), *REAP: Automatic Curation of Coding Agent Benchmarks from Interactive Production Usage*, arXiv:2604.01527; the resulting benchmark is Harvest. [practitioner / directional] Title and benchmark name verified against the arXiv record on 2026-07-29; the catalog record and RECEIPTS row 31 carry the earlier name ProdCodeBench and need the same correction.
- Bytesfortruth (2026), lending-domain benchmark account, r/compsci, 2026-03-10, https://www.reddit.com/r/compsci/comments/1rqcmu8/. [practitioner / anecdotal]
- swyx (2026), long-horizon evaluation account, X, 2026-01-14, https://x.com/swyx/status/2011344788486774942. [practitioner / anecdotal]
- Bean, A.M. et al. (2025), Measuring what Matters, arXiv:2511.04703. [lit / directional] Carried in the companion catalog as the construct-validity record; named inline for the construct-validity definition that opens this section.
