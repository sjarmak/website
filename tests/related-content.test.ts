// Unit tests for src/lib/knowledge/relatedContent.ts (PRD R5): facet->concept
...
test("relatedItems: shared-concept intersection, self-exclusion, order, per-kind cap", () => {
  const doc = (kind: RelatedDoc["kind"], id: string, concepts: string[], title = id): RelatedDoc => ({
    kind,
    id,
    title,
    url: `/${kind}/${id}`,
...
test("real data: every committed post resolves >=1 concept; software-factory talk joins", () => {
  const posts = loadPostDocs();
  assert.ok(posts.length > 0);
  for (const post of posts) {
    assert.ok(post.concepts.length >= 1, `post ${post.id} resolved no concepts`);
  }
  ...
  for (const rel of loadRelatedDocs()) {
    assert.ok(rel.concepts.length >= 1);
  }
});
