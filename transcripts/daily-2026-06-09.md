...
Here's the number that set the tone for the whole week. 8, and the group behind it, Metter, paired that with a claim that lands even harder. More than half of the passing results on SWE Bench, the benchmark the entire field has been steering on for two years, are unmergible slop, code that passes the tests but no maintainer would actually accept. So this episode is really about one thing.
...
Because we've spent two years treating pass rates as ground truth. Capcode is saying, you have to design the test so that cheating is even detectable before the number you get out of it is worth anything at all. Then there's the retrieval angle, and that comes from a benchmark called SWEE Explore. Their argument is that when you collapse a coding task down to one bit, resolved or unresolved, you throw away the part that actually predicts whether the agent succeeds.
