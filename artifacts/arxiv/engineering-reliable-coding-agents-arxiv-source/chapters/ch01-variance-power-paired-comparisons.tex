\chapter{Run-to-run variance, statistical power, and paired comparisons}\label{run-to-run-variance-statistical-power-and-paired-comparisons}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Illustrative, unmeasured geometry shows item scores for systems A and B rising together, while paired and independent mean-difference distributions share zero but pairing produces narrower uncertainty.}]{figures/ch01-paired-comparison.pdf}}
...
\section{The procedure I run before crediting a delta}\label{the-procedure-i-run-before-crediting-a-delta}
...
\subsection{Taught entry 1: never-report-a-single-run}\label{taught-entry-1-never-report-a-single-run}
...
\item
  lit/strong: Bouthillier et al.~(2021). Accounting for Variance in Machine Learning Benchmarks. MLSys 2021. arXiv:2103.03098. The \textasciitilde51x result and the fixed-seed critique.
\item
...
\subsection{Taught entry 2: power-analyze-before-running}\label{taught-entry-2-power-analyze-before-running}
...
\subsection{Taught entry 3: use-paired-tests-matched-to-metric}\label{taught-entry-3-use-paired-tests-matched-to-metric}
