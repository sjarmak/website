...
My publish-gate protocol, also an \textbf{unverified working artifact}, defines a broader intended experiment. Its first gate injects a failure in continuous integration at a randomized kill point. The fault matrix includes:
...
\section{Make every run a structured, replayable trace}\label{make-every-run-a-structured-replayable-trace}
...
\section{Benchmark recovery with fault injection}\label{benchmark-recovery-with-fault-injection}
