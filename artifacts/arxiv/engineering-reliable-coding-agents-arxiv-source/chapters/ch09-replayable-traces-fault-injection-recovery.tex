\chapter{Replayable traces and fault-injection recovery testing}\label{replayable-traces-and-fault-injection-recovery-testing}
...
The design argument in this section rests on demonstrated uses of typed traces. Yu et al.~(\href{https://arxiv.org/abs/2605.10913}{2026}) described a runtime substrate whose recorded executions can be forked and re-run from a changed step, and Zheng et al.~(\href{https://arxiv.org/abs/2508.02736}{2025}) described system-level agent observability collected below the application. Neither work compared typed traces against transcript-only observability in a controlled setting. The portability half of the argument rests on the OpenTelemetry GenAI semantic conventions (CNCF 2025), which define a shared vocabulary without testing whether adopting it improves portability.
...
The difference becomes operational during a partial failure. Suppose an agent decides at step 17 to create a support ticket. The runtime emits \texttt{tool\_call\_dispatched}, the ticket service creates ticket 8421, and the worker dies before the runtime writes \texttt{tool\_result\_persisted}. A transcript assembled after restart might omit the first call or show only an unanswered request.
...
\subsection{A shared vocabulary and its limits}\label{a-shared-vocabulary-and-its-limits}
...
\subsection{Specify the claim before the kill}\label{specify-the-claim-before-the-kill}
...
\subsection{Strike the interval the design claims to protect}\label{strike-the-interval-the-design-claims-to-protect}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Five lifecycle kill points test recovery from unstarted work through ambiguous commitment gaps to durable completion, including recognition and suppression of repeated external effects.}]{figures/ch09-kill-points.pdf}}
...
\subsection{Make every run a structured, replayable trace}\label{make-every-run-a-structured-replayable-trace}
...
\subsection{Benchmark recovery with fault injection}\label{benchmark-recovery-with-fault-injection}
