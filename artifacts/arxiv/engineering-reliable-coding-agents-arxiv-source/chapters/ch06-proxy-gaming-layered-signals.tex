...
Published support for layering comes from search-based software testing rather than from agents. Formica et al.~(\href{https://arxiv.org/abs/2207.11016}{2022}) searched Simulink models with two fitness functions, one generated from a specification and one defined by hand to encode an engineer's domain knowledge. The combined search found failures that neither guidance source found alone. That evidence is directional. It stops at complementarity in that setting and does not establish a result for coding agents.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={In illustrative shape without reported coefficients, proxy performance keeps rising as optimization pressure increases, but independent quality peaks and then declines throughout the shaded region beyond that peak.}]{figures/ch06-proxy-divergence.pdf}}
\caption{In illustrative shape without reported coefficients, proxy performance keeps rising as optimization pressure increases, but independent quality peaks and then declines throughout the shaded region beyond that peak.}
\end{figure}
...
\subsection{Layer signals beyond a single proxy}\label{layer-signals-beyond-a-single-proxy}

\begin{itemize}
\tightlist
\item
  \textbf{lit/strong:} Skalse, Howe, Krasheninnikov \& Krueger (2022). \emph{Defining and Characterizing Reward Hacking}. NeurIPS 2022. arXiv:2209.13085.
\item
  \textbf{lit/directional:} Formica, Fan \& Menghi (2022). \emph{Search-based Software Testing Driven by Automatically Generated and Manually Defined Fitness Functions}. arXiv:2207.11016.
\end{itemize}

\subsection{Monitor proxy-true divergence}\label{monitor-proxy-true-divergence}

\begin{itemize}
\tightlist
\item
  \textbf{lit/strong:} Gao, Schulman \& Hilton (2022). \emph{Scaling Laws for Reward Model Overoptimization}. ICML 2023. arXiv:2210.10760.
\item
  \textbf{lit/directional:} Laidlaw, C., et al.~(2024). \emph{Correlated Proxies}. arXiv:2403.03185.
\end{itemize}

\subsection{Monitor reasoning without training pressure}\label{monitor-reasoning-without-training-pressure}

\begin{itemize}
\tightlist
\item
  \textbf{lit/strong:} Baker, B., et al.~(2025). \emph{Monitoring Reasoning Models for Misbehavior and the Risks of Promoting Obfuscation}. OpenAI. arXiv:2503.11926.
\end{itemize}
