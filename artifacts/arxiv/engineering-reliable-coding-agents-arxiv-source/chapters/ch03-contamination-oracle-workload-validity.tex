\chapter{Benchmark contamination, oracle strength, and workload validity}\label{benchmark-contamination-oracle-strength-and-workload-validity}

In February 2026, OpenAI (\href{https://openai.com/index/why-we-no-longer-evaluate-swe-bench-verified}{Feb 2026}) stopped reporting results on a benchmark subset it had helped create. Later in July 2026, it did the same for SWE-Bench Pro (\href{https://openai.com/index/separating-signal-from-noise-coding-evaluations/}{Jul 2026}). SWE-bench Verified had become one of the most cited measures of coding-agent performance. Its 500 tasks had survived screening by 93 paid professional developers, a process intended to remove ambiguous issues and defective tests. Less than two years later, a practitioner breakdown of the sponsor's audit reported that at least 16.4 percent of the full subset was unsolvable as shipped. That figure is derived from the audited sample rather than stated in the sponsor's own summary. The same audit showed frontier models reproducing solution details verbatim when given only a task identifier.
...
Function-level code generation provides a controlled example. Liu et al.~(\href{https://arxiv.org/abs/2305.01210}{2023}) expanded a widely used suite's tests by 80x with generated inputs and mutation-based cases. Across 26 models, the stronger suite reduced \href{mailto:pass@k}{\nolinkurl{pass@k}} by up to roughly 19 to 29 percent and reshuffled rankings. The models and the generated programs were held fixed. Only the set of observations used to decide correctness changed. The original ranking had partly measured which wrong programs happened to fit sparse tests.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Stronger oracles cut SWE-ABS from 78.80\% to 62.20\% and SWE-Bench+ resolution from 12.47\% to 3.97\%; 40.9\% corrections, 29 ranking changes, 80x test expansion, 19\% to 29\% pass@k reductions, 32.67\% leakage, and 31.08\% weak-test passes.}]{figures/ch03-oracle-strength.pdf}}
\caption{Stronger oracles cut SWE-ABS from 78.80\% to 62.20\% and SWE-Bench+ resolution from 12.47\% to 3.97\%; 40.9\% corrections, 29 ranking changes, 80x test expansion, 19\% to 29\% \href{mailto:pass@k}{\nolinkurl{pass@k}} reductions, 32.67\% leakage, and 31.08\% weak-test passes.}
\end{figure}
...
Re-adjudication is the step that makes a strengthened oracle consequential. Run the stronger suite on stored patches from every system under comparison, including older submissions. Recompute \href{mailto:pass@k}{\nolinkurl{pass@k}}, rankings, paired differences, and any reward or release gate derived from the old labels. That recomputation has to start from the stored per-attempt outcomes rather than from a rescaling of the published rate, because \href{mailto:pass@k}{\nolinkurl{pass@k}} is estimated from attempts.
...
\tightlist
\item
  How old are the tasks relative to this model's training cutoff? I look for a public-versus-matched gap measured on this model. I do not substitute the leaderboard average or another model's audit.
\item
  How strong is the test oracle? I ask whether anyone augmented the tests, constructed adversarial wrong patches, or manually screened apparent successes. The useful result is the fraction of previous passes that survived.
\item
  Who produced the number, on which harness, with what task distribution? I compare the languages, prompt forms, repository structures, time horizons, context, and acceptance criteria against the work I need done.
\item
  What does the same protocol report on tasks mined from my team's merged work? I preserve the original instruction, pre-change state, acceptance evidence, and repeated-run uncertainty.
\end{enumerate}
...
\tightlist
\item
  OpenAI, ``Why we no longer evaluate SWE-bench Verified,'' 2026-02-20, \url{https://openai.com/index/why-we-no-longer-evaluate-swe-bench-verified}. {[}practitioner / corpus-verified{]}
\item
...
\subsection{Measure public-score inflation with matched controls}\label{measure-public-score-inflation-with-matched-controls}
...
\subsection{Evaluate on post-cutoff tasks}\label{evaluate-on-post-cutoff-tasks}
...
\subsection{Strengthen test oracles before adjudication}\label{strengthen-test-oracles-before-adjudication}
...
\subsection{Benchmark on your own workload}\label{benchmark-on-your-own-workload}

\begin{itemize}
\tightlist
\item
  Jha, Paltenghi, Maddila, Murali, Ugare \& Chandra (2026), \emph{REAP: Automatic Curation of Coding Agent Benchmarks from Interactive Production Usage}, arXiv:2604.01527; the resulting benchmark is Harvest. {[}practitioner / directional{]} Title and benchmark name verified against the arXiv record on 2026-07-29; the catalog record and RECEIPTS row 31 carry the earlier name ProdCodeBench and need the same correction.
\item
  Bytesfortruth (2026), lending-domain benchmark account, r/compsci, 2026-03-10, \url{https://www.reddit.com/r/compsci/comments/1rqcmu8/}. {[}practitioner / anecdotal{]}
\item
  swyx (2026), long-horizon evaluation account, X, 2026-01-14, \url{https://x.com/swyx/status/2011344788486774942}. {[}practitioner / anecdotal{]}
\item
