...
The evidence base for this chapter is narrow. It holds five items across two developed practices. Four of them are source syntheses, and none of those four is a controlled experiment. One practice has no strong evidence item. I therefore use the evidence to specify comparisons readers can run on their own workloads. The record cannot support a universal cost threshold or a recommended architecture.
...
The evidence for this practice is source synthesis, which the evidence audit retained as strong although no single controlled experiment supplies the support. The same control logic appeared independently in two corpora. In agent memory, the spine of that synthesis is SkillEvolBench, from Lei et al.~(\href{https://arxiv.org/abs/2605.24117}{2026}). In code retrieval, CoIR, from Li et al.~(\href{https://arxiv.org/abs/2407.02883}{2024}), supplies only the task and metric substrate, and the control logic below comes from the synthesis's evaluation-design blocks rather than from that benchmark paper. The provenance is imperfect, because the memory synthesis named three works behind a single carried identifier before that record was split. Two of the three are now carried at directional strength with no figure of their own. The support rests on convergence between the corpora rather than on one reported effect size.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={In this illustrative set of unmeasured configurations, five connected points form the accuracy-cost frontier and nine open points are dominated; a shaded region demonstrates domination, while minimum-accuracy and cost-ceiling guides bound feasibility.}]{figures/ch02-cost-accuracy-frontier.pdf}}
\caption{In this illustrative set of unmeasured configurations, five connected points form the accuracy-cost frontier and nine open points are dominated; a shaded region demonstrates domination, while minimum-accuracy and cost-ceiling guides bound feasibility.}
\end{figure}
...
\section{Run ablation controls}\label{run-ablation-controls}

\begin{itemize}
\tightlist
\item
  Strong evidence: \emph{SkillEvolBench} (Lei et al.~2026, arXiv:2605.24117), agentic-memory source synthesis covering no-skill and raw-trajectory controls, coverage matching, one-factor sweeps, and the writer information-flow pitfall.
\item
  Strong evidence: Evaluation-design blocks of the code-retrieval source corpus, covering the no-tool baseline, tool access as a controlled variable, and ground-truth tautology. CoIR (Li et al.~2024, arXiv:2407.02883) is the metric spine of that corpus and supplies the code-retrieval tasks and metrics only. The three controls come from the source synthesis, not from the CoIR paper.
\item
  Directional evidence: \emph{TIAP} (arXiv:2605.24060), direction only; no figure carried.
\item
  Directional evidence: \emph{MemConflict} (arXiv:2605.20926), direction only; no figure carried.
\end{itemize}

\section{Report cost-accuracy tradeoffs}\label{report-cost-accuracy-tradeoffs}
